...
export function Admin() {
  ...
  const handleAdjustMemberPoints = async (member, delta) => {
    const nextPoints = Math.max(0, member.points + delta)
    const updatedMember = await updateMember(member.email, { points: nextPoints })
    await refreshRankings()
    toast({ title: 'Points mis a jour', copy: `${member.name}: ${updatedMember?.points ?? nextPoints} points.` })
    ...
    <>
      ...
      <section className={`section admin-section motion-${motion}`}><div className="container"><div className={`admin-shell ${collapsed ? 'is-collapsed' : ''}`}>
        ...
        <div className="admin-content"><div className="admin-top"><div><p className="eyebrow">{adminTabs.find((item) => item.id === active)?.label}</p><h2>{active === 'overview' ? 'Tableau de bord' : adminTabs.find((item) => item.id === active)?.label}</h2><p>Juillet 2026 - Donnees de demonstration interactives</p></div><div className="admin-actions"><button className={`motion-control ${motion === 'boost' ? 'active' : ''}`} onClick={() => setMotion(motion === 'boost' ? 'soft' : 'boost')}>{motion === 'boost' ? '* Effets boostes' : 'o Effets doux'}</button><Button onClick={() => setNewMemberOpen(true)}>Creer un membre</Button></div></div>
          ...
          {active === 'members' && <Members rows={filteredMembers} query={query} setQuery={setQuery} onRemove={removeMember} onPasswordReset={handleResetMemberPassword} onPointsAdjust={handleAdjustMemberPoints} />}
...
function Members({ rows, query, setQuery, onRemove, onPasswordReset, onPointsAdjust }) {
  ...
  const selectedPointsMember = rows.find((member) => member.email === pointsTarget)
  ...
  const openPointsAdjust = (email) => {
    setPointsTarget(email)
    setPointsAmount('')
  }

  const submitPointsAdjust = async (event, direction) => {
    event.preventDefault()
    if (!selectedPointsMember) return

    const amount = Math.floor(Number(pointsAmount))
    if (!Number.isFinite(amount) || amount <= 0) return

    const delta = direction === 'add' ? amount : -amount
    await onPointsAdjust(selectedPointsMember, delta)
    setPointsTarget('')
    setPointsAmount('')
  }

  return <div className="admin-view fade-enter"><div className="toolbar"><div><h3>Membres de la communaute</h3><p>Creation, gestion des roles et suivi des points.</p></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un membre..." /></div>{selectedMember && <form className="admin-card password-reset-card" onSubmit={submitPasswordReset}><div><p className="eyebrow">Reset acces</p><h3>Changer le mot de passe</h3><p>{selectedMember.name} pourra se connecter avec ce mot de passe temporaire.</p></div><label className="field"><span>Nouveau mot de passe</span><input type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="Mot de passe temporaire" autoFocus /></label><div className="form-actions"><Button type="submit">Enregistrer</Button><Button type="button" variant="secondary" onClick={() => { setPasswordTarget(''); setNewPassword('') }}>Annuler</Button></div></form>}{selectedPointsMember && <form className="admin-card password-reset-card" onSubmit={(event) => submitPointsAdjust(event, 'add')}><div><p className="eyebrow">Ajustement points</p><h3>Ajouter ou retirer des points</h3><p>{selectedPointsMember.name} a actuellement {selectedPointsMember.points} points.</p></div><label className="field"><span>Montant</span><input type="number" min="1" step="1" value={pointsAmount} onChange={(event) => setPointsAmount(event.target.value)} placeholder="ex. 10" autoFocus /></label><div className="form-actions"><Button type="submit">Ajouter</Button><Button type="button" variant="danger" onClick={(event) => submitPointsAdjust(event, 'remove')}>Retirer</Button><Button type="button" variant="secondary" onClick={() => { setPointsTarget(''); setPointsAmount('') }}>Annuler</Button></div></form>}<div className="admin-card data-table members-table"><div className="data-head"><span>MEMBRE</span><span>ROLE</span><span>POINTS</span><span>STATUT</span><span>ACTIONS</span></div>{rows.map((member) => <div className="data-row" key={member.email}><span className="member-cell"><i>{member.name[0]}</i><b>{member.name}<small>{member.email}</small></b></span><Pill tone={member.role === 'Admin' ? 'purple' : 'muted'}>{member.role}</Pill><strong>{member.points}</strong><Pill tone={member.status === 'Actif' ? 'success' : 'warning'}>{member.status}</Pill><span className="row-buttons"><button className="row-action" onClick={() => openPointsAdjust(member.email)}>Points</button><button className="row-action" onClick={() => openPasswordReset(member.email)}>Changer MDP</button><button className="row-action danger" onClick={() => onRemove(member)}>Retirer</button></span></div>)}</div></div>
}
